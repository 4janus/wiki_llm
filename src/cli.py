...
Available commands:
  ...
  topics        key theme collection and normalization
  groups        group pages from document metadata
...
def _get_opts(ctx: typer.Context, stages: list[str]) -> tuple[WikiConfig, PipelineOptions]:
    state = ctx.ensure_object(dict)
    cfg = _load_config(state.get("config_path", _DEFAULT_CONFIG))
...
def _run(ctx: typer.Context, stages: list[str]) -> None:
    cfg, opts = _get_opts(ctx, stages)
...
def generate(ctx: typer.Context) -> None:
    ...
    _run(ctx, ["read", "generate"])


@app.command()
def topics(ctx: typer.Context) -> None:
    """Collect terms from wiki pages, normalize via LLM, and generate key theme pages."""
    _run(ctx, ["key_themes"])


@app.command()
def groups(ctx: typer.Context) -> None:
    """Generate group pages from document metadata."""
    cfg, _ = _get_opts(ctx, [])
...
def repair(ctx: typer.Context) -> None:
    ...
    _run(ctx, ["lint", "repair"])


@app.command(name="run-all")
def run_all(ctx: typer.Context) -> None:
    """Run all pipeline stages in order."""
    _run(ctx, ["read", "generate", "key_themes", "groups", "index", "consolidate", "lint", "repair"])
