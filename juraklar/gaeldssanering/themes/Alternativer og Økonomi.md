# Tema: Alternativer og Økonomi

## Oversigt

Dette tema samler ressourcer om alternativer til gældssanering samt de økonomiske og skattemæssige konsekvenser af gældssanering.

## Centrale Spørgsmål dette Tema Dækker

- Hvad er alternativerne til gældssanering?
- Hvornår er frivillig akkord bedre?
- Hvad er de skattemæssige konsekvenser?
- Hvad sker der med SU-gæld?
- Hvad sker der med skattegæld?
- Hvad sker der med kreditvurderingen?
- Hvad sker der med gæld stiftet under saneringen?

## Nøgleressourcer

### Artikler
- [Alternativer vejledning](../articles/alternativer-vejledning.md) — sammenligning af løsninger

### Raw Materiale
- [Alternativer til gældssanering](../raw/alternativer-til-gaeldssanering.md) — oversigt over alternativer

### Politikker
- [Skattemæssige konsekvenser](../policies/skattemassige-konsekvenser.md) — skattebehandling ved gældssanering og akkord

## Alternativer — Sammenligning

| Alternativ | Bedst til | Kræver |
|------------|-----------|--------|
| Frivillig akkord | Få, samarbejdsvillige kreditorer | Alle kreditorers accept |
| SKAT-aftale | Primært skattegæld | Kontakt til SKAT |
| Salg af aktiver | Ejendom/bil med friværdi | Salgbare aktiver |
| Gieldsrådgivning | Alle situationer | Kommunal service (gratis) |
| Gieldskonsolidering | Mild gieldssituation | Kreditværdighed |

## Skattemæssige Nøglepunkter

| Situation | Skattepligtig? |
|-----------|---------------|
| Gæld eftergivet via Skifterettens kendelse | NEJ — altid skattefri |
| Frivillig akkord af hensyn til betalingsevne | NEJ — skattefri |
| Frivillig akkord af forretningsmæssige årsager | JA — potentielt skattepligtig |
| Gield der afdrages normalt | Ikke relevant |

## Særlige Gieldstyper

### SU-Gæld
- Kan som udgangspunkt **ikke** saneres
- Særregler gælder
- Dispensation mulig i særlige tilfælde (varig uarbejdsdygtighed)

### Skattegæld
- Kan saneres som øvrig usikret gæld
- SKAT behandles som øvrig kreditor
- SKAT modtager dividenden

### Pantesikret Gæld
- Realkreditlån og pantebrev — IKKE omfattet af saneringen
- Skyldner betaler videre som normalt
- Ejendom beholdes, hvis ydelse betales

### Kautionsgæld
- Kautionsforpligtelse kan i visse tilfælde saneres
- Kautionisten skal selv opfylde betingelserne

## Kreditvurdering

Gældssanering medfører registrering i **RKI/Experian** som følge af misligholdelse inden saneringen. Registreringen slettes normalt 5 år efter oprettelsen eller ved betaling/afsluttet sag.

Gældssaneringskendelsen offentliggøres i Statstidende, hvilket er offentligt tilgængeligt.

## Relaterede Temaer

- [Gældssanering og Betingelser](./Gældssanering og Betingelser.md)
- [Saneringsprocessen](./Saneringsprocessen.md)

## Relevante Rådgivningsservices

- Service 12: Hvad sker der med min SU-gæld?
- Service 13: Hvad sker der skattemæssigt?
- Service 14: Hvad er alternativer til gældssanering?
- Service 15: Kan gæld til SKAT saneres?
- Service 20: Hvad er konsekvenser for min kreditvurdering?
- Service 21: Hvad er frivillig akkord og hvornår er det bedre?
- Service 22: Hvad sker der med ny gæld under saneringsperioden?
