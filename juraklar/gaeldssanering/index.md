# Gieldssanering

Denne wiki dækker gieldssanering i Danmark — den retslige ordning der giver privatpersoner med håbløs gæld mulighed for et nyt start via Skifteretten.

## Hvad er Gieldssanering?

Gieldssanering er en ordning hjemlet i **konkurslovens kapitel 25-29**, der giver privatpersoner mulighed for delvis eftergivelse af gæld via en kendelse fra Skifteretten. Skyldner betaler et fast månedligt beløb i 5 år — resten eftergives. Ordningen binder alle usikrede kreditorer og er skattefri.

## Kom Hurtigt i Gang

| Spørgsmål | Ressource |
|-----------|-----------|
| Hvad er gieldssanering? | [Artikel: Hvad er gieldssanering](articles/hvad-er-gaeldssanering.md) |
| Kan jeg søge? | [Artikel: Betingelser vejledning](articles/betingelser-vejledning.md) |
| Hvordan søger jeg? | [Artikel: Processen trin for trin](articles/processen-trin-for-trin.md) |
| Hvad skal jeg betale? | [Artikel: Saneringsforslag og budget](articles/saneringsforslag-og-budget.md) |
| Er der alternativer? | [Artikel: Alternativer vejledning](articles/alternativer-vejledning.md) |

## Mappestruktur

```
gaeldssanering/
├── raw/                        Rådata og lovtekster
│   ├── konkursloven-gaeldssanering.md
│   ├── betingelser-og-undtagelser.md
│   ├── processen-hos-skifteretten.md
│   ├── saneringsforslag-og-raadighetsbelob.md
│   └── alternativer-til-gaeldssanering.md
├── policies/                   Politikker og lovgrundlag
│   ├── konkurslov-kapitel-25-29.md
│   └── skattemassige-konsekvenser.md
├── articles/                   Vejledningsartikler
│   ├── hvad-er-gaeldssanering.md
│   ├── betingelser-vejledning.md
│   ├── processen-trin-for-trin.md
│   ├── saneringsforslag-og-budget.md
│   └── alternativer-vejledning.md
├── notes/                      Juridiske noter
│   └── juridiske-grundbegreber-gaeldssanering.md
├── themes/                     Temaer
│   ├── Gældssanering og Betingelser.md
│   ├── Saneringsprocessen.md
│   └── Alternativer og Økonomi.md
├── groups/                     Grupper
│   ├── lovgivning.md
│   ├── vejledninger.md
│   └── grundbegreber.md
└── prompts/                    Rådgivningsservices (25 stk.)
    ├── 01-hvad-er-gaeldssanering-og-hvem-kan-soege.md
    ├── 02-betingelserne-for-gaeldssanering.md
    └── ... (25 services i alt)
```

## Temaer

- [Gieldssanering og Betingelser](themes/Gældssanering og Betingelser.md) — definition, betingelser, hvem kan søge
- [Saneringsprocessen](themes/Saneringsprocessen.md) — processen, saneringsforslag, kreditorerne
- [Alternativer og Økonomi](themes/Alternativer og Økonomi.md) — alternativer, skat, særlige gieldstyper

## Lovgrundlag

- Konkursloven kap. 25-29 (lbkg. nr. 11/2014)
- Retsplejeloven (Skifterettens kompetence)
- Ligningsloven § 24 (skattefrihed)
- Lov om gieldsrådgivning

## Rådgivningsmenu

Se [Rådgivningsmenu](raadgivningsmenu.md) for alle 25 rådgivningsservices.

## Topics

Se [Topics](topics.md) for emneoversigt.

## Lint Rapport

Se [Lint rapport](lint_report.md).
